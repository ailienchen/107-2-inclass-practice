...
```{r ans13}
grade*sumEcon
meanEcon <- sum(grade*sumEcon)/sum(sumEcon)
meanEcon
```
```{r}
...
library(lubridate)
submitTime
strDates <- c("submitTime")
dates <- as.Date(submitTime, "%m/%d/%Y") 
dates
```
...
nightOwl <- hour_twTime >=1 & hour_twTime<=6
nightOwl
```
...
library(tidyverse)
transcriptDataFinal%>%
  filter(studentID=="046S6f645" & schoolYear=="100")->st100
  studentID <- st100$學號
  schoolYear <- st100$學年
  grade <- st100$學期成績
  credit <- st100$學分數
...
credit100ChosenStudent <- sum(credit)
...
library(tidyverse)
transcriptDataFinal%>%
  filter(studentID=="046S6f645" & schoolYear=="100"& grade>="60")->st100
  studentID <- st100$學號
  schoolYear <- st100$學年
  grade <- st100$學期成績
  credit <- st100$學分數
...
earnedCredit100ChosenStudent <- sum(credit)
...
library(tidyverse)
transcriptDataFinal%>%
  filter(studentID=="046S6f645" & schoolYear=="100"& credit>"0")->st100
studentID <- st100$學號
schoolYear <- st100$學年
grade <- st100$學期成績
credit <- st100$學分數
...
gpa100ChosenStudent<-(sum((credit*grade))/sum(credit))
